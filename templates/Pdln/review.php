	...
	<div id="kt_content_container" class="container-fluid">
		<div class="card">
			...
			<div class="row">
				...
				<div class="col-md-12">
					...
					<div class="card-body">
						...
						<div class="row mb-7">
							<label class="col-lg-2 fw-bold text-muted">Unit Pengusul :</label><br />
							<div class="col-lg-10">
								<span class="fw-bolder fs-6 text-gray-800" id="nama_lengkap"><?=$data_detail->user->name?></span>
							</div>
							<div class="col-lg-10">
								<span class="fw-bolder fs-6 text-gray-800" id="nama_lengkap"><?=$data_detail->user->unit?></span>
							</div>
						...
						<div class="row mb-7">
							<label class="col-lg-2 fw-bold text-muted">Tanggal Pengajuan :</label><br />
							<div class="col-lg-10">
			...
			<div class="card-body pt-5">
                ...
                // Add icon-based approval buttons
										$approvalName = $doc['name'] . '_approval';
										echo '<td>
										<center>
										<button type="button" class="btn btn-success btn-sm" 
										id="' . $approvalName . '_approve" 
										onclick="toggleApproval(\'' . $approvalName . '\', 1)">
										<i class="bi bi-check-circle"></i>
										</button>
										<button type="button" class="btn btn-danger btn-sm" 
										id="' . $approvalName . '_reject" 
										onclick="toggleApproval(\'' . $approvalName . '\', 2)">
										<i class="bi bi-x-circle"></i>
										</button>
										<input type="text" name="' . $approvalName . '" id="' . $approvalName . '" value="0">
										</center>
										</td>';
						...
						<script>
    // JavaScript function to handle the toggle behavior of approval/reject buttons
							function toggleApproval(inputName, value) {
								const approveButton = document.getElementById(inputName + '_approve');
								const rejectButton = document.getElementById(inputName + '_reject');
								const hiddenInput = document.getElementById(inputName);

						        if (value === 1) { // If Approve button is clicked
						        	if (hiddenInput.value !== "1") {
						        		hiddenInput.value = "1";
						                rejectButton.style.display = 'none'; // Hide Reject button
						            } else {
						            	hiddenInput.value = "0";
						                rejectButton.style.display = ''; // Show Reject button
						            }
						        } else if (value === 2) { // If Reject button is clicked
						        	if (hiddenInput.value !== "2") {
						        		hiddenInput.value = "2";
						                approveButton.style.display = 'none'; // Hide Approve button
						            } else {
						            	hiddenInput.value = "0";
						                approveButton.style.display = ''; // Show Approve button
						            }
						        }
						    }
						</script>
							...
							?>
							    <div class="card">
								        <div class="card-body">
								            ...
								            <div class="mb-5">
								            	<?php if ($data_detail->approval_status === 'pending'): ?>
					                                <span class="badge bg-warning">Pengajuan Baru</span>
					                            <?php elseif ($data_detail->approval_status === 'setuju'): ?>
					                                <span class="badge bg-success">Disetujui</span>
					                            <?php elseif ($data_detail->approval_status === 'tidak_setuju'): ?>
					                            ...
					                            <Br />
					                            <small><?php
					                            $date = new DateTime($data_detail->approval_date);
					                            $approvalDateTime = $date->format("d-m-Y H:i:s");

					                            echo $approvalDateTime;
					                              
					                              ?></small><br />
					                              Keterangan :<br />
					                            <?=$data_detail->approval_notes?>
								            </div>
								        </div>
								    </div>
